...
nair = n_air(lam_of_interest, t, p)
a3 = 0
b3 = beta_air(lam_of_interest, t, p)

n = n2
root1 = sqrt(n**2 - sin(theta_t)**2)
rootair = sqrt(nair**2 - sin(theta_t)**2)

m1 = 2 * asarray([[n * sqrt(1 - (sin(theta_t) / n)**2), -nair * sqrt(1 - (sin(theta_t) / nair)**2)],
                  [-1 / (2 * root1), 1 / (2 * rootair)]])
v1 = asarray([fopd, 0])

thickness = la.solve(m1, v1)
print(thickness, [n_ref_nsf66, n_ref_psf68, nair], [n1, n2, nair])
